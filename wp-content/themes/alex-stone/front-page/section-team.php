<div class="front_page_section front_page_section_team<?php
			$alex_stone_scheme = alex_stone_get_theme_option('front_page_team_scheme');
			if (!alex_stone_is_inherit($alex_stone_scheme)) echo ' scheme_'.esc_attr($alex_stone_scheme);
			echo ' front_page_section_paddings_'.esc_attr(alex_stone_get_theme_option('front_page_team_paddings'));
		?>"<?php
		$alex_stone_css = '';
		$alex_stone_bg_image = alex_stone_get_theme_option('front_page_team_bg_image');
...
?>><?php
	// Add anchor
	$alex_stone_anchor_icon = alex_stone_get_theme_option('front_page_team_anchor_icon');	
	$alex_stone_anchor_text = alex_stone_get_theme_option('front_page_team_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_team_inner<?php
			if (alex_stone_get_theme_option('front_page_team_fullheight'))
				echo ' alex_stone-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$alex_stone_css = '';
			$alex_stone_bg_mask = alex_stone_get_theme_option('front_page_team_bg_mask');
			$alex_stone_bg_color = alex_stone_get_theme_option('front_page_team_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$alex_stone_caption = alex_stone_get_theme_option('front_page_team_caption');
			if (!empty($alex_stone_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo !empty($alex_stone_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses($alex_stone_caption, 'alex_stone_kses_content'); ?></h2><?php
			}
		
			// Description (text)
			$alex_stone_description = alex_stone_get_theme_option('front_page_team_description');
			if (!empty($alex_stone_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo !empty($alex_stone_description) ? 'filled' : 'empty'; ?>"><?php echo wpautop(wp_kses($alex_stone_description, 'alex_stone_kses_content')); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_team_output"><?php 
				if (is_active_sidebar('front_page_team_widgets')) {
					...
					if (!alex_stone_exists_trx_addons())
						...
						alex_stone_customizer_need_widgets_message('front_page_team_caption', 'ThemeREX Addons - Team');
				}
			?></div>
		</div>
	</div>
