...
if (!function_exists('alex_stone_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'alex_stone_hovers_theme_setup3', 3 );
	function alex_stone_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		alex_stone_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'alex-stone'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'alex-stone') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'alex-stone')
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'alex-stone'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'alex-stone') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Default',	'alex-stone')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('alex_stone_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'alex_stone_hovers_theme_setup9', 9 );
	function alex_stone_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'alex_stone_hovers_frontend_scripts', 1010 );
		add_filter( 'alex_stone_filter_localize_script','alex_stone_hovers_localize_script' );
		add_filter( 'alex_stone_filter_merge_scripts',	'alex_stone_hovers_merge_scripts' );
		add_filter( 'alex_stone_filter_merge_styles',	'alex_stone_hovers_merge_styles' );
		add_filter( 'alex_stone_filter_get_css', 		'alex_stone_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'alex_stone_hovers_frontend_scripts' ) ) {
	
	function alex_stone_hovers_frontend_scripts() {
		if ( alex_stone_is_on(alex_stone_get_theme_option('debug_mode')) && alex_stone_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'alex-stone-hovers', alex_stone_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( alex_stone_is_on(alex_stone_get_theme_option('debug_mode')) && alex_stone_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'alex-stone-hovers',  alex_stone_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('alex_stone_hovers_merge_scripts')) {
	
	function alex_stone_hovers_merge_scripts($list) {
...
if (!function_exists('alex_stone_hovers_merge_styles')) {
	
	function alex_stone_hovers_merge_styles($list) {
...
if (!function_exists('alex_stone_hovers_localize_script')) {
	
	function alex_stone_hovers_localize_script($arr) {
		$arr['button_hover'] = alex_stone_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('alex_stone_hovers_add_icons') ) {
	function alex_stone_hovers_add_icons($hover, $args=array()) {
			...
			<div class="icons">
				<?php
					...
					if (!empty($attachment[0]))
						$large_image = $attachment[0];
				?><?php if (!empty($large_image)) { ?>
				<a href="<?php echo esc_url($large_image); ?>" aria-hidden="true" class="shop_link_zoom button icon-magnifying-search-lenses-tool" title="<?php the_title_attribute(); ?>"></a><?php } ?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link2"><span><?php
					esc_html_e('Details', 'alex-stone');
				?></span></a>
...
// Add styles into CSS
if ( !function_exists( 'alex_stone_hovers_get_css' ) ) {
	
	function alex_stone_hovers_get_css($css, $colors, $fonts, $scheme='') {
