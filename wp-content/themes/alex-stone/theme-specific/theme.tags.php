...
if ( !function_exists('alex_stone_need_page_title') ) {
	function alex_stone_need_page_title() {
		return !is_front_page() && apply_filters('alex_stone_filter_need_page_title', true);
	}
}
...
if ( !function_exists('alex_stone_show_layout') ) {
	function alex_stone_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('alex_stone_get_logo_image') ) {
	function alex_stone_get_logo_image($type='') {
		$logo_image = '';
		if (alex_stone_is_on(alex_stone_get_theme_option('logo_retina_enabled')) && alex_stone_get_retina_multiplier(2) > 1)
			$logo_image = alex_stone_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = alex_stone_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('alex_stone_get_header_video') ) {
	function alex_stone_get_header_video() {
		$video = '';
		if (apply_filters('alex_stone_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (alex_stone_check_theme_option('header_video')) {
				$video = alex_stone_get_theme_option('header_video');
...
if ( !function_exists('alex_stone_show_post_meta') ) {
	function alex_stone_show_post_meta($args=array()) {

		if (is_single() && alex_stone_is_off(alex_stone_get_theme_option('show_post_meta'))) return ' ';	// Space is need!
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post categories
				if ($comp == 'categories') {
					$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('alex_stone_filter_get_post_categories', '');
					if (!empty($cats)) {
						?>
						<span class="post_meta_item post_categories"><?php alex_stone_show_layout($cats); ?></span>
						<?php
					}

				// Post date
				} else if ($comp == 'date') {
					$dt = apply_filters('alex_stone_filter_get_post_date', alex_stone_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post counters
					...
					if ( ($output = alex_stone_get_post_counters($args['counters'])) != '')
						alex_stone_show_layout($output);
					else {
						if (!is_singular() || have_comments() || comments_open()) {
							...
							<a href="<?php echo esc_url(get_comments_link()); ?>" class="post_meta_item post_counters_item post_counters_comments icon-comment-light"><?php
								...
								?><span class="post_counters_label"><?php
									echo esc_html(_n('Comment', 'Comments', $post_comments, 'alex-stone'));
								?></span>
							</a>
							<?php
						}
				...
				// Socials share
					...
					alex_stone_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'alex-stone'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'alex-stone' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('alex_stone_show_post_featured') ) {
	function alex_stone_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => alex_stone_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : alex_stone_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? alex_stone_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = alex_stone_get_post_image();
				if (!empty($image)) 
					$image = alex_stone_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . alex_stone_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured post_attachment<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>">
					...
					if (!$args['thumb_bg']) 
						echo wp_get_attachment_image( get_the_ID(), $thumb_size, false, 
													 alex_stone_is_on(alex_stone_get_theme_option('seo_snippets'))
				...
				?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$alex_stone_attr = alex_stone_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($alex_stone_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($alex_stone_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						alex_stone_hovers_add_icons($args['hover']);
					}
					...
					if (($output=alex_stone_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						alex_stone_show_layout($output);
					...
					if (!empty($image)) {
						...
						alex_stone_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = alex_stone_get_post_video($post_content, false);
						if (empty($video))
							$video = alex_stone_get_post_iframe($post_content, false);
						if (empty($video)) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($videos) && is_array($videos)) {
									$video = alex_stone_array_get_first($videos, false);
								}
							}
						}
						if (!empty($video)) {
							if ( $has_thumb ) {
								$video = alex_stone_make_video_autoplay($video);
								?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
							}
							?><div class="post_video video_frame"><?php 
								if ( !$has_thumb ) {
									alex_stone_show_layout($video);
								}
							?></div><?php
						}
					...
					// Put audio over the thumb
						...
						$audio = alex_stone_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = alex_stone_get_post_iframe($post_content, false);
						if (empty($audio)) {
							// Only get video from the content if a playlist isn't present.
							$post_content = apply_filters( 'the_content', get_the_content() );
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($audios) && is_array($audios)) {
									$audio = alex_stone_array_get_first($audios, false);
								}
							}
						}
						if (!empty($audio)) {
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
								...
								$media_author = alex_stone_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = alex_stone_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !alex_stone_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php alex_stone_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !alex_stone_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php alex_stone_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								alex_stone_show_layout($audio); 
							?></div><?php
						}
					}
				}
				...
				alex_stone_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				alex_stone_show_layout($post_info);
...
if ( !function_exists('alex_stone_get_no_image') ) {
	function alex_stone_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = alex_stone_get_theme_option( 'no_image' );
			if (empty($img)) $img = alex_stone_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('alex_stone_add_bg_in_post_nav') ) {
	function alex_stone_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = alex_stone_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), alex_stone_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), alex_stone_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'alex-stone-main', $css );
	}
}

// Show related posts
if ( !function_exists('alex_stone_show_related_posts') ) {
	function alex_stone_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : alex_stone_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'alex-stone');
				?></h3>
...
if ( !function_exists('alex_stone_show_portfolio_posts') ) {
	function alex_stone_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? alex_stone_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = alex_stone_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = alex_stone_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the alex_stone_ajax_get_posts action
if ( !function_exists( 'alex_stone_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_alex_stone_ajax_get_posts',			'alex_stone_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_alex_stone_ajax_get_posts',	'alex_stone_ajax_get_posts_callback');
	function alex_stone_ajax_get_posts_callback() {
		if ( !wp_verify_nonce( alex_stone_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			wp_die();
	
		$id = !empty($_REQUEST['blog_template']) ? alex_stone_get_value_gpc('blog_template') : 0;
		if ($id > 0) {
			alex_stone_storage_set('blog_archive', true);
			alex_stone_storage_set('blog_mode', 'blog');
			alex_stone_storage_set('options_meta', get_post_meta($id, 'alex_stone_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => alex_stone_show_portfolio_posts(array(
							'cat' => (int) alex_stone_get_value_gpc('cat'),
							'parent_cat' => (int) alex_stone_get_value_gpc('parent_cat'),
							'page' => (int) alex_stone_get_value_gpc('page'),
							'post_type' => trim(alex_stone_get_value_gpc('post_type')),
							'taxonomy' => trim(alex_stone_get_value_gpc('taxonomy')),
							'blog_style' => trim(alex_stone_get_value_gpc('blog_style')),
							'echo' => false
							)
						)
		);
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'alex-stone');
		}
...
// Show pagination
if ( !function_exists('alex_stone_show_pagination') ) {
	function alex_stone_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = alex_stone_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'alex-stone' ),
				'next_text' => esc_html__( '>', 'alex-stone' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'alex-stone' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'alex-stone'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'alex-stone') : esc_html__('Newest posts', 'alex-stone') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'alex-stone') : esc_html__('Older posts', 'alex-stone'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'alex_stone_single_comments_field' ) ) {
    function alex_stone_single_comments_field( $args ) {
        ...
        if ( 'text' == $args['field_type'] ) {
            ...
            $html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
                    . ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'alex_stone_kses_content' ) . '</label>';
        } else {
        ...
        if ( 'default' != $args['form_style'] ) {
            $html .= '<span class="sc_form_field_hover">'
                        ...
                        . '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'], 'alex_stone_kses_content' ) . '</span>'
                    . '</span>';
        }
        $html .= '</span></div>';
        return $html;
