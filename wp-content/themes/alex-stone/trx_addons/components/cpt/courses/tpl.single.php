...
while ( have_posts() ) { the_post();
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class( 'courses_single itemscope' ); trx_addons_seo_snippets('', 'Article'); ?>>
		...
		<div class="courses_column_image">
				<?php
				// Image
				...
				?>
		</div><div class="courses_column_content">
			<div class="courses_page_header">	
				<?php
				...
				if ( !trx_addons_sc_layouts_showed('postmeta') ) {
					?><div class="courses_page_meta">
						<span class="courses_page_meta_item courses_page_meta_date"><?php
							$dt = $meta['date'];
							echo sprintf($dt < date('Y-m-d') ? esc_html__('Started on %s', 'alex-stone') : esc_html__('Starting %s', 'alex-stone'), '<span class="courses_page_meta_item_date">' . date(get_option('date_format'), strtotime($dt)) . '</span>');
						?></span>
		...
		?><div class="courses_page_content entry-content"<?php trx_addons_seo_snippets('articleBody'); ?>><?php
			...
			if (!empty($meta['product']) && (int) $meta['product'] > 0) {
				...
				?>
				<a href="<?php echo esc_url(get_permalink($meta['product'])) ?>" class="theme_button"><?php echo esc_html__('Join the Course', 'alex-stone') ?></a>
				<?php
			}
