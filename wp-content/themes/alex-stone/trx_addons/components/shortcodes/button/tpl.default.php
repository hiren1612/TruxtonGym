...
?><div class="sc_item_button sc_button_wrap<?php if (!trx_addons_is_off($args['align'])) echo ' sc_align_'.esc_attr($args['align']); ?>"><?php
	...
	?><a href="<?php echo esc_url($args['link']); ?>"<?php
		...
		?> class="sc_button sc_button_<?php
				...
				if (($args['inverse_button'] > 0)) echo ' inverse_button'; 
				...
				if (empty($args['image']) && !empty($args['icon'])) echo ' sc_button_with_icon';
				?>"<?php
	...
	if (!empty($args['image']) || !empty($args['icon'])) {
		?><span class="sc_button_icon"><?php
			if (!empty($args['image'])) {
				...
				?><img class="sc_icon_as_image" src="<?php echo esc_url($args['image']); ?>" alt="<?php echo esc_attr(basename($args['image'])); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			} else if (trx_addons_is_url($args['icon'])) {
				$attr = trx_addons_getimagesize($args['icon']);
				?><img class="sc_icon_as_image" src="<?php echo esc_url($args['icon']); ?>" alt="<?php echo esc_attr(basename($args['icon'])); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
